...
class FocalLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        loss *= self.__alpha * th.pow(th.abs(target - th.sigmoid(input)), self.__gamma)
        return loss
...
class YoloV3Loss(nn.Module):
    ...
    def __cal_loss_per_layer(self, p, p_d, label, bboxes, stride):
        ...
        # loss giou
        giou = tools.GIOU_xywh_th(p_d_xywh, label_xywh).unsqueeze(-1)
        ...
        # loss confidence
        iou = tools.iou_xywh_th(p_d_xywh.unsqueeze(4), bboxes.unsqueeze(1).unsqueeze(1).unsqueeze(1))
        ...
        loss_giou = (th.sum(loss_giou)) / batch_size
        loss_conf = (th.sum(loss_conf)) / batch_size
        loss_cls = (th.sum(loss_cls)) / batch_size
...
if __name__ == "__main__":
    ...
    import darknet53
    net = darknet53.DARKNET53()
    p_d = net(th.rand(3, 3, 416, 416))
    label_sbbox = th.rand(3,  52, 52, 3,26)
    label_mbbox = th.rand(3,  26, 26, 3, 26)
    label_lbbox = th.rand(3, 13, 13, 3,26)
    sbboxes = th.rand(3, 150, 4)
    mbboxes = th.rand(3, 150, 4)
    lbboxes = th.rand(3, 150, 4)
